...
$count = mysql_num_rows(mysql_query("SELECT*FROM loaiphong"));
...
if (isset($_REQUEST['list_id']) && $_REQUEST['list_id'] != '') {
	...
	for ($i = 0; $i < count($del_pic_id); $i++) {
		...
		$sql = "SELECT * FROM loaiphong WHERE id = '" . $del_pic_id[$i] . "'";
	...
	mysql_query('DELETE FROM loaiphong WHERE loaiphong.id IN(' . $list_id . ')');

?>

	<script>
		window.location = "quan_tri.php?p=danhsach_loaiphong"
	</script>
...
if (isset($_REQUEST['noibat']) && $_REQUEST['noibat'] != '') {
	...
	if ($noibat == 0) {
		...
		mysql_query("update loaiphong set noibat=1 where id=" . $id . "");
	}

	if ($noibat == 1) {

		mysql_query("update loaiphong set noibat=0 where id=" . $id . "");
	}
...

{
...

	{
		...
		if (($curpage != 1) && ($curpage)) {
			...
			$page_list .= "<a href=\"" . $_SERVER['PHP_SELF'] . "?p=danhsach_loaiphong&page=1\" title=\"Trang đầu\" style='margin:4px;'> << </a>";
		}

		if ($curpage - 1 > 0) {

			$page_list .= "<a href=\"" . $_SERVER['PHP_SELF'] . "?p=danhsach_loaiphong&page=" . ($curpage - 1) . "\" title=\"Về trang trước\" style='margin:4px;'><font color='#00ccff'> < </font></a>";
		}
		...
		for ($i = 1; $i <= $pages; $i++) {
			...
			if ($i == $curpage) {
				...
				$page_list .= "<a href=\"" . $_SERVER['PHP_SELF'] . "?p=danhsach_loaiphong&page=" . $i . "\" title=\"Trang " . $i . "\" style='margin:4px;'><font color='red'>[" . $i . "]</font></a>";
			}
		...
		if (($curpage + 1) <= $pages) {
			...
			$page_list .= "<a href=\"" . $_SERVER['PHP_SELF'] . "?p=danhsach_loaiphong&page=" . ($curpage + 1) . "\" title=\"Đến trang sau\" style='margin:4px;' ><font color='#00ccff'> > </font></a>";
		}

		if (($curpage != $pages) && ($pages != 0)) {

			$page_list .= "<a href=\"" . $_SERVER['PHP_SELF'] . "?p=danhsach_loaiphong&page=" . $pages . "\" title=\"Trang cuối\" > >> </a>";
		}
...

	{
		...
		if (($curpage - 1) < 0 || ($curpage - 1) != 1) {
			...
			$next_prev = "<a href=\"?p=danhsach_loaiphong&page=" . ($curpage - 1) . "\">Back</a>";
		}
		...
		if (($curpage + 1) > $page) {
			...
			$next_prev = "<a href=\"?p=danhsach_loaiphong&page=" . ($curpage + 1) . "\">Next</a>";
		}
...
$result = mysql_query("SELECT * FROM loaiphong $loai ORDER BY id DESC limit $start,$limit");
...
?>
	...
	<form action="quan_tri.php?p=danhsach_loaiphong" method="post">
...
<?
		...
		echo  "<td align='center'><a href=\"quan_tri.php?p=xoa_loaiphong&id=" . $id . "\" ­ onclick=\"return confirm('Bạn có muốn xóa thông tin này ?')\"><img src='images/xoa_record.png' border='0'></a></td>";

		echo  "<td align='center'><a href='quan_tri.php?p=sua_loaiphong&id=" . $id . "&khachsan=" . $khachsan . "'><img src='images/sua_record.png' border='0'></a></td>";
