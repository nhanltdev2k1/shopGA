<?
  ...
  $count=mysql_num_rows(mysql_query("SELECT*FROM traloi"));
  ...
  if(isset($_REQUEST['list_id']) && $_REQUEST['list_id'] != ''){
		...
		for($i = 0; $i < count($del_pic_id); $i++){
			...
			$sql = "SELECT * FROM traloi WHERE id = '" . $del_pic_id[$i] . "'";
		...
		mysql_query('DELETE FROM traloi WHERE binhchon.id IN(' . $list_id . ')');
	...
	if($noibat==0){
	 ...
	 mysql_query("update traloi set trangthai=1 where id=".$id."");

	}

	if($noibat==1){

	 mysql_query("update traloi set trangthai=0 where id=".$id."");
...
function delete_all_chosen(){
	...
	window.location ="quan_tri.php?p=danhsach_traloi&list_id=" + list_id;
...

	{
...

		{
...

			{
				...
				$page_list.="<a href=\"".$_SERVER['PHP_SELF']."?p=traloi&page=1\" title=\"Trang đầu\" style='margin:4px;'> << </a>";
...

			{
				...
				$page_list.="<a href=\"".$_SERVER['PHP_SELF']."?p=traloi&page=".($curpage-1)."\" title=\"Về trang trước\" style='margin:4px;'><font color='#00ccff'> < </font></a>";
...
<?php
...

	{
	  ...
	  $result=mysql_query("SELECT * FROM traloi ORDER BY id DESC limit $start,$limit");
     ...
     echo "<h2 align='center'>Danh Sách Trả Lời Bình Luận </h2>";
    ...
    <?
	   ...
	   while($row = mysql_fetch_object($result))
		 ...
		 $did=$row->user;
	      ...
	      $sql3=mysql_query("select * from binhchon where id='".$did."'");
...
?>
		...
		<a href="quan_tri.php?p=traloi&trangthai=0&id=<?=$id?>">No</a>
		...
		<a href="quan_tri.php?p=traloi&trangthai=1&id=<?=$id?>">Active</a>
...
<?
			...
			echo  "<td align='center'><a href=\"quan_tri.php?p=xoa_traloi&id=".$id."\" ­ onclick=\"return confirm('Bạn có muốn xóa thông tin này ?')\"><img src='images/xoa_record.png' border='0'></a></td>";

		echo  "<td align='center'><a href='quan_tri.php?p=sua_traloi&id=".$id."&khachsan=".$khachsan."'><img src='images/sua_record.png' border='0'></a></td>";
