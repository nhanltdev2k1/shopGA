...
<table width="780" height="145" border="1"  style="border-collapse:collapse">
  ...
  <tr>
	...
	if($_GET['id']!=''){
	 $mySql=mysql_query("select * from danhmuc where id=".$_GET['id']."");
  ...
  if(isset($_POST['luu']))
   {
	...
	$pro_id= $_POST['pro_id'];
	$tieude=$_POST['tieude'];
    upload($id,$pro_id,$tieude);
   }
   
function upload($id,$pro_id,$tieude)
 {
   $ketnoi_maychu = ketnoi_MC();
	chon_CSDL($ketnoi_maychu);
   $truyvan = " update danhmuc set thuocloai=".$pro_id.",tieude='".$tieude."' where id=".$id."";
...
function xoataptin($tentaptin)
 {
   $ketnoi_maychu = ketnoi_MC();
	...
	$truyvan = "DELETE FROM danhmuc WHERE id = $masotaptin ";
	$ketqua_truyvan = truyvan($truyvan,$ketnoi_maychu);
 }
?></td>
    <td valign="top"><p><br />
  ...
  <tr>
    <td height="66"  colspan="2" align="center" valign="top"><form action="" method="post" enctype="multipart/form-data" name="tt_mh" id="tt_mh"  onsubmit="return checkInput();">
      <table width="780" border="1" bordercolor="black" style="border-collapse:collapse; border-color:#FF0000">
        <tr>
          <td width="233" valign="top"><div align="left"><strong>Sản phẩm </strong></div></td>
          <td width="531"><div align="left">
            <label>
            <select name="pro_id">
              <?php
			  $selected=""; 
			  $sql=mysql_query("select * from loai_ma_sanpham where id=".$_GET['pro_id']."");
			  $num=mysql_num_rows($sql);			  
			  if($_GET['pro_id']!='' && $num>0){
			   $sql=mysql_query("select * from loai_ma_sanpham where id=".$_GET['pro_id']."");
			   $selectecd="selected";
			   }else{
			   $sql=mysql_query("select * from loai_ma_sanpham");
			   }
			   while($pro_id=mysql_fetch_array($sql)){
			   ?>
               <option value="<?php echo $pro_id['id'];?> <?php echo $selected;?>"><?php echo $pro_id['thuocloai'];?></option>
               <?php
			   }			  
			  ?>
              </select>
        ...
        <tr>
          <td valign="top"><div align="left"><strong>Tiêu đề</strong></div></td>
          <td><div align="left">
            <label>
              <input name="tieude" type="text" id="tieude2" size="60"  maxlength="70" value="<?php echo $suasize['tieude'];?>"/>
            </label>
          </div></td>
        </tr>
